...
import { loadDay, loadHabits, migrateLocal, loadGoals, loadQuits } from './firestore.js';
...
export function observeAuthState(onSignIn, onSignOut) {
  return onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      const goals = await loadGoals(user.uid);
      const quits = await loadQuits(user.uid);
      const todayData = await loadDay(user.uid);
      if (onSignIn) onSignIn(user, { habits, goals, quits, day: todayData });
    } else if (onSignOut) {
      onSignOut();
    }
  });
