...
import {
  ...
  addTask as addTaskRemote,
  removeTask as removeTaskRemote,
  addJournal as addJournalRemote,
  removeJournal as removeJournalRemote,
  addDream as addDreamRemote,
  removeDream as removeDreamRemote,
  addIdea as addIdeaRemote,
  removeIdea as removeIdeaRemote,
...
(function () {
  ...
  const hydrateFromRemote = (payload = {}) => {
    ...
    // Destructure incoming payload with sane defaults
    const { habits = [], day = {}, goals = [], quits = [] } = payload;
    // Map habits to local format (handling Firestore timestamps)
    state.habits = habits.map((h) => ({
      ...
      created: h.createdAt?.toDate
        ? dayKey(h.createdAt.toDate())
        : dayKey(h.createdAt || h.created || todayKeyValue)
    }));
    // Persist goals list
    state.goals = goals.map((g) => {
      const created =
        g.created?.toDate && typeof g.created.toDate === 'function'
          ? g.created.toDate().getTime()
          : g.created ?? Date.now();
      return { id: g.id, title: g.title, created };
    });
    // Persist quits list
    state.quits = quits.map((q) => {
      const created =
        q.created?.toDate && typeof q.created.toDate === 'function'
          ? q.created.toDate().getTime()
          : q.created ?? Date.now();
      // Firestore stores date string for quit date
      return { id: q.id, name: q.name, date: q.date || todayKeyValue, created };
    });
    // Merge completions for today
    const completions = day.completions || {};
    const dayState = getDay(todayKeyValue);
    dayState.habits = { ...dayState.habits, ...completions };
    // Copy remote arrays into local day state
    if (Array.isArray(day.tasks)) dayState.tasks = day.tasks.slice();
    if (Array.isArray(day.journal)) dayState.journal = day.journal.slice();
    if (Array.isArray(day.dreams)) dayState.dreams = day.dreams.slice();
    if (Array.isArray(day.ideas)) dayState.ideas = day.ideas.slice();
  ...
  const renderTasks = () => {
    ...
    day.tasks.forEach((task) => {
      ...
      remove.addEventListener('click', async () => {
        // Persist removal remotely
        if (currentUser && currentUser.uid) {
          try {
            await removeTaskRemote(currentUser.uid, today(), task.id);
          } catch (e) {
            console.warn('Failed to remove task remotely', e);
          }
        }
    ...
    const buildRow = (quit, allowReset = false) => {
      ...
      if (allowReset) {
        ...
        reset.addEventListener('click', async () => {
          ...
          const newDate = startOfDayIso();
          // Persist remotely
          if (currentUser && currentUser.uid) {
            try {
              await updateQuitRemote(currentUser.uid, quit.id, { date: newDate });
            } catch (e) {
              console.warn('Failed to update quit remotely', e);
            }
          }
          quit.date = newDate;
          saveState();
      ...
      if (!sortedGoals.length) {
        ...
        sortedGoals.forEach((goal) => {
          ...
          edit.addEventListener('click', async () => {
            ...
            if (currentUser && currentUser.uid) {
              try {
                await updateGoalRemote(currentUser.uid, goal.id, { title: trimmed });
              } catch (e) {
                console.warn('Failed to update goal remotely', e);
              }
            }
            goal.title = trimmed;
            saveState();
          ...
          remove.addEventListener('click', async () => {
            if (currentUser && currentUser.uid) {
              try {
                ...
                console.warn('Failed to remove goal remotely', e);
              }
            }
  ...
  if (dom.habitDate) {
    dom.habitDate.addEventListener('change', () => {
  ...
  if (dom.taskForm) {
    dom.taskForm.addEventListener('submit', async (event) => {
      ...
      if (!title) return;
      const newTask = { id: randomId(), title, created: Date.now() };
      // Persist remotely if user is signed in
      if (currentUser && currentUser.uid) {
        try {
          await addTaskRemote(currentUser.uid, today(), newTask);
        } catch (e) {
          console.warn('Failed to save task remotely', e);
        }
      }
      const day = getDay(today());
  ...
  if (dom.goalForm) {
    dom.goalForm.addEventListener('submit', async (e) => {
      ...
      if (currentUser && currentUser.uid) {
        try {
          goalId = await addGoalRemote(currentUser.uid, text);
        } catch (error) {
          console.warn('Failed to add goal remotely', error);
        }
      }
      state.goals.push({ id: goalId, title: text, created: Date.now() });
      dom.goalInput.value = '';
      saveState();
      renderGoals();
      renderDashboardSummary();
  ...
  if (dom.quitForm) {
    dom.quitForm.addEventListener('submit', async (e) => {
      ...
      if (currentUser && currentUser.uid) {
        try {
          qid = await addQuitRemote(currentUser.uid, name, storedDate);
        } catch (error) {
          console.warn('Failed to add quit remotely', error);
        }
      }
      state.quits.push({ id: qid, name, date: storedDate });
      dom.quitName.value = '';
  ...
  if (dom.journalForm) {
    dom.journalForm.addEventListener('submit', async (event) => {
      ...
      // Persist remotely if signed in
      if (currentUser && currentUser.uid) {
        try {
          await addJournalRemote(currentUser.uid, today(), entry);
        } catch (e) {
          console.warn('Failed to save journal remotely', e);
        }
      }
      const day = getDay(today());
  ...
  if (dom.dreamForm) {
    dom.dreamForm.addEventListener('submit', async (event) => {
      ...
      if (currentUser && currentUser.uid) {
        try {
          await addDreamRemote(currentUser.uid, today(), entry);
        } catch (e) {
          console.warn('Failed to save dream remotely', e);
        }
      }
      const day = getDay(today());
  ...
  if (dom.ideaForm) {
    dom.ideaForm.addEventListener('submit', async (event) => {
      ...
      if (currentUser && currentUser.uid) {
        try {
          await addIdeaRemote(currentUser.uid, today(), entry);
        } catch (e) {
          console.warn('Failed to save idea remotely', e);
        }
      }
      const day = getDay(today());
